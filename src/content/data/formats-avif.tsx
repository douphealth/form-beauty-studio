...
export const FORMAT_CONTENT: Record<string, ContentEntry> = {
  "/formats/avif": {
    ...
    sections: [
      ...
      { id: "encoding-speed", heading: "Encoding speed", body: [P(<>AVIF's AV1-based encoder is slower than WebP's — often 3–10× for a single image. ImageAlchemy runs AVIF encoding in background Web Workers with pause, resume and cancel, so batches never block the interface.</>)] },
      { id: "how-to-convert", heading: "How to convert to AVIF with ImageAlchemy", body: [P(<>1. Drop your files. 2. Choose <strong>AVIF</strong> as the output format. 3. Set quality (60–70 recommended) and max dimension. 4. Compress, then download individually or as a ZIP. Up to 200 images per batch, fully on-device.</>)] },
    ],
    faqs: [
      { question: "How do I convert an image to AVIF?", answer: "Drop your files into ImageAlchemy, select AVIF as the output format, set quality (60–70 recommended, since AVIF's scale differs from JPEG and WebP) and any max dimension, then compress and download. Everything runs in your browser; up to 200 images per batch." },
      ...
      { question: "Why is AVIF encoding slow?", answer: "AVIF uses the AV1 video codec's encoder, which is computationally heavier than WebP's or JPEG's. ImageAlchemy handles this by encoding in background WebAssembly workers, so you can pause, resume and cancel while the interface stays responsive." },
    ],
    related: [
